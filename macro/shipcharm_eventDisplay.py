...
# must be old data, don't expect hit at 0,0,0  
         hitlist[(p.GetZ(),p.GetTime())] = [p.GetX(),p.GetY()]
        else:   
         hitlist[(lp.z(),100)] = [lp.x(),lp.y()] 
         hitlist[(2.*p.GetZ()-lp.z(),100)] = [2.*p.GetX()-lp.x(),2.*p.GetY()-lp.y()] 
       else:
        hitlist[(p.GetZ(),p.GetTime())] = [p.GetX(),p.GetY()]
   if len(hitlist)==1:
...
# sort in z
   ...
   if len(lz)>1:
    lz.sort(key=lambda lz:lz[1]) #sorting by time 
    for z in lz:  DTrack.SetNextPoint(hitlist[z][0],hitlist[z][1],z[0])
    p = pdg.GetParticle(fT.GetPdgCode()) 
    if p : pName = p.GetName()
    else:  pName =  str(fT.GetPdgCode())
