//MuonTaggerHit class, derived from muonHit class (on date 5 April 2018)
...
bool MuonTaggerHit::onlyOnce=false;
const Double_t tileXdim = 1., tileYdim = 1.; // single tile dimension
...
// -----   Default constructor   -------------------------------------------
MuonTaggerHit::MuonTaggerHit()
  : ShipHit()
{
}
// -----   Standard constructor   ------------------------------------------
MuonTaggerHit::MuonTaggerHit(Int_t detID, Float_t digi, Bool_t isV)
  : ShipHit(detID,digi) 
{
  if (!onlyOnce) {
    stInit();
...
MuonTaggerHit::MuonTaggerHit(MuonTaggerPoint* p, Double_t t0)
  : ShipHit()
{
  if (!onlyOnce) {
    stInit();
...
// ----
Int_t MuonTaggerHit::DetIDfromXYZ(TVector3 p)
{  
...
// ----
TVector3 MuonTaggerHit::XYZfromDetID(Int_t dID)
{
//
...
//
    for (Int_t i = 0; i < nodes->GetSize(); i++) {
      ...
      Int_t muStNs = 0;
      if (TString(node->GetName()).Contains("Sensitive")) {
...
// -------------------------------------------------------------------------
Double_t MuonTaggerHit::SetMuonTimeRes(Double_t mcTime) {
//
...
void MuonTaggerHit::Print() const {
//
  cout << "-I- MuonTaggerHit: muon tagger hit " << " in detector " << fDetectorID << endl;
  cout << "  TDC " << fdigi << " ns" << endl;

}
void MuonTaggerHit::setValidity(Bool_t isV){hisV = isV;}
// -----   Destructor   ----------------------------------------------------
MuonTaggerHit::~MuonTaggerHit() { }
// -------------------------------------------------------------------------
